...
for i in range(1, 10):
    ...
    KMeansModel.fit(dataFrame_scaled)  # Fitting the data to the model
...
KMeansModel.fit(dataFrame_scaled)  # Fitting the data to the model
clusters = KMeansModel.predict(dataFrame_scaled)  # Predicting the cluster for each sample.
# silhouette_score indicates how dense the data is with in the cluster range from -1 to 1.
print("The Silhouette score is: ", silhouette_score(dataFrame_scaled, clusters))
# The score is 0.197 which indicates the model is performing good but not good as the previous model.
# The standardization decreased the score value.
